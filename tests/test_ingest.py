...
class TestIngestEntities:
    async def test_adds_new_entities(self, store: MemoryGraphStore):
        result = _result(entities=[("Alex", "person"), ("Python", "tool")])
        stats = await ingest_extraction(store, result)
        assert stats["nodes_added"] == 2
        assert store.node_count == 2

    async def test_deduplicates_by_name(self, store: MemoryGraphStore):
        ...
        stats = await ingest_extraction(store, result2)

        assert stats["nodes_added"] == 1  # Only Lena is new
        assert store.node_count == 2

    async def test_deduplication_case_insensitive(self, store: MemoryGraphStore):
    ...
    async def test_maps_entity_types(self, store: MemoryGraphStore):
        ...
        alex = (await store.find_nodes(name_contains="Alex"))[0]
        python = (await store.find_nodes(name_contains="Python"))[0]
        tokyo = (await store.find_nodes(name_contains="Tokyo"))[0]
    ...
    async def test_empty_result(self, store: MemoryGraphStore):
        stats = await ingest_extraction(store, _result())
        assert stats == {"nodes_added": 0, "edges_added": 0, "edges_skipped": 0}


class TestIngestRelations:
    async def test_adds_edges(self, store: MemoryGraphStore):
        result = _result(
    ...
    async def test_auto_creates_unknown_source(self, store: MemoryGraphStore):
        result = _result(
    ...
    async def test_auto_creates_unknown_target(self, store: MemoryGraphStore):
        result = _result(
    ...
    async def test_resolves_entities_case_insensitive(self, store: MemoryGraphStore):
        result = _result(
            entities=[("User", "person"), ("Python", "tool")],
            relations=[("user", "python", "prefers", 0.9)],
        )
    ...
    async def test_multiple_relations(self, store: MemoryGraphStore):
        result = _result(
...
class TestIngestAcrossMessages:
    ...
    async def test_graph_grows_across_messages(self, store: MemoryGraphStore):
        # Message 1
        r1 = _result(
            entities=[("User", "person"), ("Alex", "person")],
            relations=[("User", "Alex", "named", 0.95)],
        )
        await ingest_extraction(store, r1)
        assert store.node_count == 2
        assert store.edge_count == 1
        ...
        stats2 = await ingest_extraction(store, r2)
        ...
        stats3 = await ingest_extraction(store, r3)
