...
async def test_dedup_reuses_node_by_exact_name_case_insensitive(store):
    ...
    await ingest_extraction(store, result1)
    await ingest_extraction(store, result2)
    assert store.node_count == 1


async def test_dedup_reuses_node_from_store_across_sessions(store):
...
async def test_dedup_does_not_create_duplicate_when_name_exists(store):
    result1 = ExtractionResult(
    ...
    await ingest_extraction(store, result1)
    await ingest_extraction(store, result2)
    nodes = await store.find_nodes(name_contains="Graph Theory")
    assert len(nodes) == 1


async def test_dedup_merges_properties_on_existing_node(memory_store):
    ...
    await memory_store.add_node(node)
    ...
    await ingest_extraction(memory_store, result)

    node_data = await memory_store.get_node("t1")
...
async def test_dedup_emits_node_updated_on_reuse(memory_store):
...
async def test_dedup_emits_node_added_on_new_node(store):
    ...
    await ingest_extraction(store, result)
...
async def test_dedup_local_index_takes_priority_over_store(store):
    """The local name_to_id index (this ingestion pass) should be checked first."""
    # Pre-populate store with a node
    await store.add_node(make_node("Alpha", NodeType.ENTITY, node_id="store_alpha"))

    local_index = {"alpha": "local_alpha"}
    result = await _resolve_entity_name("Alpha", store, local_index)
...
async def test_update_node_properties_merges_correctly(memory_store):
    node = make_node("TestNode", NodeType.ENTITY, node_id="t1", key1="val1")
    await memory_store.add_node(node)
    await memory_store.update_node_properties("t1", {"key2": "val2"})

    data = await memory_store.get_node("t1")
...
async def test_update_node_properties_new_key_wins(memory_store):
    node = make_node("TestNode", NodeType.ENTITY, node_id="t1", status="old")
    await memory_store.add_node(node)
    await memory_store.update_node_properties("t1", {"status": "new"})
...
async def test_update_node_properties_noop_for_unknown_id(memory_store):
    """Updating a nonexistent node should be a no-op."""
    await memory_store.update_node_properties("nonexistent", {"key": "val"})
