...
async def test_ingest_maps_theorem_type_correctly(store):
    ...
    nodes = await store.find_nodes(node_type=NodeType.THEOREM)
    assert len(nodes) == 1
    assert nodes[0]["name"] == "Euler's Theorem"


async def test_ingest_maps_paper_type_correctly(store):
    from neuroweave.extraction.pipeline import ExtractedEntity, ExtractionResult
    ...
    await ingest_extraction(store, result)
    nodes = await store.find_nodes(node_type=NodeType.PAPER)
    assert len(nodes) == 1
...
def _build_math_graph(store: MemoryGraphStore) -> None:
    ...
    GraphStore.add_node(store, t1)
    GraphStore.add_node(store, t2)
    GraphStore.add_node(store, l1)
    GraphStore.add_node(store, d1)
    GraphStore.add_node(store, p1)

    GraphStore.add_edge(store, make_edge("t1", "l1", "uses", 0.95, edge_id="e1"))
    GraphStore.add_edge(store, make_edge("t1", "d1", "belongs_to", 0.99, edge_id="e2"))
    GraphStore.add_edge(store, make_edge("t2", "d1", "belongs_to", 0.99, edge_id="e3"))
    GraphStore.add_edge(store, make_edge("p1", "t1", "proves", 0.95, edge_id="e4"))


async def test_query_by_type_returns_theorems_only(store):
    _build_math_graph(store)
    result = await query_by_type(store, NodeType.THEOREM)
...
async def test_query_by_type_with_relation_filter(store):
    _build_math_graph(store)
    result = await query_by_type(store, NodeType.THEOREM, relations=["uses"])
    assert any(e["relation"] == "uses" for e in result.edges)
    assert not any(e["relation"] == "belongs_to" for e in result.edges)


async def test_get_proof_chain_traverses_uses_relation(store):
    _build_math_graph(store)
    result = await get_proof_chain(store, "Four Color Theorem")
    assert len(result.nodes) > 0
    relations = {e["relation"] for e in result.edges}
    assert relations.issubset({"uses", "follows_from", "proves", "verified_by"})


async def test_get_domain_graph_returns_members(store):
    _build_math_graph(store)
    result = await get_domain_graph(store, "Graph Theory")
