...
@pytest.fixture
def e2e_store() -> MemoryGraphStore:
...
class TestConversationBuildsGraph:
    ...
    async def test_graph_is_not_empty(self, e2e_result: MemoryGraphStore):
        assert e2e_result.node_count > 0
        assert e2e_result.edge_count > 0

    async def test_expected_node_count(self, e2e_result: MemoryGraphStore):
    ...
    async def test_expected_edge_count(self, e2e_result: MemoryGraphStore):
...
class TestPeopleExtracted:
    ...
    async def test_alex_exists(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="Alex")
        assert len(matches) >= 1

    async def test_lena_exists(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="Lena")
        assert len(matches) >= 1

    async def test_user_exists(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="User")
        assert len(matches) >= 1

    async def test_children_exist(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="children")
        assert len(matches) >= 1
...
class TestPlacesAndConcepts:
    ...
    async def test_tokyo_exists(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="Tokyo")
        assert len(matches) == 1
        assert matches[0]["node_type"] == NodeType.ENTITY.value

    async def test_python_exists(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="Python")
        assert len(matches) == 1

    async def test_sushi_exists(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="sushi")
        assert len(matches) >= 1

    async def test_ramen_exists(self, e2e_result: MemoryGraphStore):
        matches = await e2e_result.find_nodes(name_contains="ramen")
        assert len(matches) >= 1
...
class TestRelationships:
    ...
    async def test_married_to_relation(self, e2e_result: MemoryGraphStore):
        edges = await e2e_result.get_edges(relation="married_to")
        assert len(edges) >= 1
        edge = edges[0]
        assert edge["confidence"] >= 0.85

    async def test_traveling_to_relation(self, e2e_result: MemoryGraphStore):
        edges = await e2e_result.get_edges(relation="traveling_to")
        assert len(edges) >= 1  # At least User → Tokyo

    async def test_food_preferences(self, e2e_result: MemoryGraphStore):
        prefs = await e2e_result.get_edges(relation="prefers")
        assert len(prefs) >= 2  # Lena→sushi, User→ramen
        targets = {e["target_id"] for e in prefs}
        # Resolve target names
        target_names = set()
        for tid in targets:
            node = await e2e_result.get_node(tid)
            if node:
                target_names.add(node["name"].lower())
        assert "sushi" in target_names or "ramen" in target_names

    async def test_has_children_relation(self, e2e_result: MemoryGraphStore):
        edges = await e2e_result.get_edges(relation="has_children")
        assert len(edges) >= 1
        assert edges[0]["confidence"] >= 0.85

    async def test_python_experience(self, e2e_result: MemoryGraphStore):
        edges = await e2e_result.get_edges(relation="experienced_with")
        assert len(edges) >= 1
        edge = edges[0]
        assert edge["confidence"] >= 0.85
...
class TestConfidenceScores:
    ...
    async def test_all_confidences_in_range(self, e2e_result: MemoryGraphStore):
        data = await e2e_result.to_dict()
        for edge in data["edges"]:
            assert 0.0 <= edge["confidence"] <= 1.0, (
                f"Edge {edge['relation']} has confidence {edge['confidence']} out of range"
            )

    async def test_explicit_facts_high_confidence(self, e2e_result: MemoryGraphStore):
        ...
        for rel in high_confidence_relations:
            edges = await e2e_result.get_edges(relation=rel)
            for edge in edges:
                assert edge["confidence"] >= 0.85, (
...
class TestGraphStructure:
    ...
    async def test_no_orphan_edges(self, e2e_result: MemoryGraphStore):
    ...
    async def test_serialization_roundtrip(self, e2e_result: MemoryGraphStore):
    ...
    async def test_user_is_central_node(self, e2e_result: MemoryGraphStore):
        """User should be the most connected node in the graph."""
        user_nodes = await e2e_result.find_nodes(name_contains="User")
        assert len(user_nodes) >= 1
        user_id = user_nodes[0]["id"]
        neighbors = await e2e_result.get_neighbors(user_id, depth=1)
